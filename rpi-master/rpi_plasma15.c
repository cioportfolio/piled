...
#define LED_NCHANS      1   // Number of LED channels (8 or 16)
...
#define CHAN_MAXLEDS    (NUM_LEDS / LED_NCHANS)  // Maximum number of LEDs per channel
#define CHASE_MSEC      6 // Delay time for chaser light test
...
int main(int argc, char *argv[])
{
    ...
    if (testmode)
    {
        while (1)
        {
            ...
            driftFactor = 0; //sinf(timeFactor/5.0);
...
float soid1( float x, float y)
{
  return sinf(x * (10.0 + driftFactor * 2.5) + timeFactor + 10.0*driftFactor);
}
